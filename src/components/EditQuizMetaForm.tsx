...
export function EditQuizMetaForm({ title, author, description, error, onSave }: EditQuizMetaFormProps) {
  ...
  if (!open) return <button type="button" className="secondary" onClick={openForm}>✏️ Modifier les informations du quiz</button>

  return <div className="question-edit-form">
    <label>Titre<input type="text" value={draftTitle} onChange={(event) => setDraftTitle(event.target.value)} /></label>
    <label>Auteur<input type="text" value={draftAuthor} onChange={(event) => setDraftAuthor(event.target.value)} /></label>
    <label>Description<textarea value={draftDescription} onChange={(event) => setDraftDescription(event.target.value)} /></label>
    {error && <p className="alert" role="alert">{error}</p>}
    <div className="question-edit-actions">
      <button type="button" onClick={save} disabled={saving || !draftTitle.trim() || !draftAuthor.trim()}>Enregistrer</button>
      <button type="button" className="secondary" onClick={() => setOpen(false)} disabled={saving}>Annuler</button>
    </div>
  </div>
}
