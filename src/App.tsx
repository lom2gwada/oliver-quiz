...
export default function App() {
  ...
  return <main className="app-shell">
    <header><div><p className="eyebrow">QUIZ TECHNIQUE</p><h1>{quiz.metadata.title}</h1><p>par {quiz.metadata.author}</p></div><label className="file-input">Importer un JSON<input type="file" accept="application/json,.json" onChange={(event) => loadFile(event.target.files?.[0])} /></label></header>
    ...
    {view === 'quiz' && <QuizPage quiz={quiz} questions={sessionQuestions} onFinish={(nextAnswers) => { setAnswers(nextAnswers); setView('results') }} />}
    {view === 'results' && <ResultPage questions={sessionQuestions} answers={answers} onRestart={() => { setAnswers({}); setSessionQuestions([]); setView('start') }} />}
  </main>
}
